...
% describing the background, problem, strategy, metrics, activities, result analysis, lessons learned, 
% follow up actions, and summary/conclusions. A high level summary or an abstract should also be 
...
% Please note: This it is a "report” and not a collection of data and models without description and discussion.
...
\begin{itemize}
    \item \textbf{Lines of Code (LOC):} Total, source, comment, and blank lines measured via tools like cloc or tokei
    \item \textbf{Cyclomatic Complexity:} Code path complexity measured via Radon (Python) or similar language-specific analyzers
    \item \textbf{Code Coverage:} Percentage of code exercised by tests, extracted from pytest-cov or coverage.py reports
    \item \textbf{Technical Debt:} Estimated remediation time calculated via SonarQube or CodeClimate analysis
    \item \textbf{Maintainability Index:} Composite metric combining complexity, LOC, and comment density
\end{itemize}
...
\begin{enumerate}
    \item \textbf{Repository Modification Requirements:} Adding custom GitHub Actions workflows requires forking repositories or obtaining maintainer approval to modify .github/workflows/ directory
    \item \textbf{Complexity for Established Projects:} Large, active projects like LangChain have existing CI/CD pipelines; integrating custom actions risks conflicts with established workflows
    \item \textbf{Maintainer Approval Barriers:} External researchers cannot reasonably request that production repositories add custom observability workflows for academic studies
    \item \textbf{Scalability Concerns:} Each target repository would require custom workflow configuration tailored to its language, build system, and testing framework
\end{enumerate}
...
\begin{enumerate}
    \item \textbf{OpenTelemetry Collector (otelcol-contrib):} Scrapes GitHub repositories every 60 seconds using the GitHub receiver with Personal Access Token (PAT) authentication via bearer token extension
    \item \textbf{OpenObserve:} Stores time-series metrics on port 5080, receiving OTLP metrics at \texttt{/api/default/} endpoint with SQL-based querying for dashboard visualization
    \item \textbf{Docker Networking:} Bridge network (\texttt{otel-network}) with persistent storage (\texttt{openobserve-data} volume) for metric retention
\end{enumerate}

\begin{figure}[H]
    \centering
    % TODO: Add architecture diagram showing OTel Collector -> OpenObserve pipeline
    \caption{OpenTelemetry DORA Metrics Architecture (Phase 2 Approach)}
    \label{fig:otel-architecture}
\end{figure}

The GitHub receiver supports flexible search queries (e.g., \texttt{org:<ORG>}, \texttt{is:private}, \texttt{is:pr is:open}) enabling targeted collection across repositories and pull request states.
...
\begin{itemize}
    \item \texttt{vcs.change.time\_to\_merge} - Duration from PR creation to merge (seconds)
    \item \texttt{vcs.change.time\_to\_approval} - Duration from PR creation to approval (seconds)
...
\begin{itemize}
    ...
    \item \texttt{vcs.ref.lines\_delta} - Code churn (lines added/removed) vs. trunk
...
\begin{enumerate}
    \item \textbf{GitHub API Authentication:} GitHub PAT tokens require specific repository scopes (\texttt{repo}, \texttt{read:org}) and organization approval for accessing organizational repositories
    \item \textbf{Permission Model Constraints:} Public repositories provide limited data through unauthenticated API access; detailed PR metrics require authenticated requests
    \item \textbf{Rate Limiting:} GitHub API rate limits (5,000 requests/hour for authenticated users, 60/hour for unauthenticated) constrained continuous polling across multiple repositories
    \item \textbf{Organizational Barriers:} Accessing repositories under organizational accounts (langchain-ai, intel) requires PAT tokens approved by organization administrators
    \item \textbf{Data Completeness Gaps:} Change Failure Rate calculation required GitHub Actions workflow status integration, which was not available through the standard GitHub receiver
\end{enumerate}
...
\begin{itemize}
    \item \textbf{Universal Access:} No authentication requirements or organizational approvals needed
    \item \textbf{Data Availability:} Public repositories expose comprehensive PR and issue history through web UI
    \item \textbf{DORA Compatibility:} Sufficient data granularity to calculate industry-standard DORA metrics
    \item \textbf{Reproducibility:} Any researcher can replicate this methodology for public repositories
\end{itemize}
...
\begin{enumerate}
    \item \textbf{Deployment Frequency (DF):} How often an organization successfully releases to production (proxied by merged PR rate)
    \item \textbf{Lead Time for Changes (LT):} The time it takes for a commit to get into production (measured as PR open-to-merge duration)
    \item \textbf{Change Failure Rate (CFR):} The percentage of deployments causing a failure in production (calculated from bug issue frequency)
    \item \textbf{Time to Restore Service (MTTR):} How long it takes to recover from a failure in production (measured as bug report to fix merge duration)
\end{enumerate}
...
\begin{itemize}
    \item \textbf{Pull Request Events:} Creation timestamps, merge timestamps, and closure timestamps for all PRs
    \item \textbf{Issue Events:} Creation timestamps and issue titles/descriptions containing bug-related keywords
    \item \textbf{Metadata:} Commit hashes, relative dates (e.g., "2 days ago"), and absolute timestamps
\end{itemize}
...
\begin{itemize}
    \item \textbf{Workflow Integration Complexity:} Adding custom GitHub Actions to repositories with existing CI/CD pipelines risks breaking production workflows
    \item \textbf{Maintainer Approval Requirements:} External researchers cannot reasonably request that active projects modify their infrastructure for academic studies
    \item \textbf{Forking Limitations:} Forked repositories do not reflect real-world development practices and lose access to organizational resources
\end{itemize}
...
\begin{itemize}
    \item \textbf{Organizational Permissions:} GitHub's permission model requires organization-level approval for PAT tokens accessing organizational repositories, creating a gatekeeping mechanism that external researchers cannot bypass
    \item \textbf{Rate Limiting at Scale:} GitHub's API rate limits (5,000 requests/hour authenticated, 60/hour unauthenticated) make continuous monitoring of multiple active repositories impractical without organizational quotas
    \item \textbf{Scope Creep in Permissions:} Obtaining sufficient API access requires broad permission scopes (\texttt{repo}, \texttt{read:org}) that organizations rightfully restrict to trusted members
\end{itemize}
...
\begin{enumerate}
    \item \textbf{Phase 1 (GitHub Actions):} Maximum data richness (code-level metrics) but maximum invasiveness
    \item \textbf{Phase 2 (OpenTelemetry):} Moderate data richness (VCS metrics) but still blocked by permissions
    \item \textbf{Phase 3 (Manual Collection):} Minimum data richness (PR/issue metadata) but universal accessibility
\end{enumerate}
...
\begin{itemize}
    \item \textbf{Scope Reduction:} Moving from internal code metrics to external process metrics preserved analytical value while reducing technical requirements
    \item \textbf{Pragmatic Compromise:} Manual data collection, though labor-intensive, proved more feasible than complex automation infrastructure
    \item \textbf{Research Question Adaptation:} Shifting focus to DORA metrics aligned with industry-recognized frameworks, enhancing result interpretability
\end{itemize}
...
\begin{itemize}
    \item \textbf{Industry Standardization:} DORA metrics provide benchmarked performance classifications, enabling meaningful comparisons without domain-specific baselines
    \item \textbf{Process-Level Insights:} While missing code-level detail, DORA metrics capture team velocity, quality practices, and operational maturity
    \item \textbf{Publicly Derivable:} All four DORA metrics can be calculated from publicly visible GitHub data, enabling reproducible research
    \item \textbf{Actionable Framework:} DORA's elite/high/medium/low classifications provide clear performance assessment without complex interpretation
\end{itemize}
...
\begin{enumerate}
    \item \textbf{Start with minimal access assumptions:} Design data collection strategies assuming no repository access, authentication, or organizational cooperation
    \item \textbf{Prioritize non-invasive observation:} Favor passive data collection over active instrumentation to avoid modifying target systems
    \item \textbf{Leverage public observability:} GitHub's public Insights, PR history, and issue tracking provide substantial data without authentication
    \item \textbf{Use industry-standard frameworks:} DORA, SPACE, and other recognized frameworks enhance result interpretability and benchmarking
    \item \textbf{Automate analysis, not collection:} When collection must be manual, invest in automated analysis pipelines for reproducibility
    \item \textbf{Build incremental validation:} Test approaches on personal/accessible repositories before attempting large-scale collection
    \item \textbf{Document methodological pivots:} Transparency about failed approaches provides value to future researchers facing similar constraints
\end{enumerate}
