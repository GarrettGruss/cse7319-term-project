...
The intended workflow would have been added to target repositories at \texttt{.github/workflows/quality-metrics.yml}:
...
The workflow would have used \texttt{python-semantic-release} for automated version management:
...
\subsection{Abandonment Rationale - Detailed}

This comprehensive GitHub Actions approach was abandoned due to several critical barriers:

\begin{enumerate}
    \item \textbf{Repository Modification Complexity}
    \begin{itemize}
        \item Adding workflows requires write access to \texttt{.github/workflows/} directory
        \item Target repositories (LangChain, Intel XPU Backend) have existing CI/CD infrastructure
        \item Risk of conflicts with existing workflows, branch protection rules, and required checks
        \item Need to modify \texttt{.gitignore}, add \texttt{pyproject.toml}, and create metrics directories
    \end{itemize}

    \item \textbf{Maintainer Approval Barriers}
    \begin{itemize}
        \item External researchers cannot submit PRs that add observability workflows for academic purposes
        \item Maintainers would reject workflows that export metrics to external platforms (security/privacy)
        \item Request would require detailed justification, privacy impact assessment, and legal review
        \item No precedent for academic metric collection in production repositories
    \end{itemize}

    \item \textbf{Per-Repository Customization Requirements}
    \begin{itemize}
        \item Different projects use different languages (Python, C++, Rust), requiring tool variations
        \item Test frameworks vary: pytest, unittest, cargo test, gtest, etc.
        \item Build systems differ: setuptools, poetry, cargo, CMake, requiring custom metric extraction
        \item Each repository would need custom quality gate thresholds based on project maturity
    \end{itemize}

    \item \textbf{Fork Limitations}
    \begin{itemize}
        \item Forking allows workflow addition but creates isolation from production development
        \item Forks do not receive upstream commits in real-time, metrics become stale
        \item Fork metrics do not reflect actual project health, only snapshot quality
        \item Cannot access organizational secrets, runners, or internal deployment metrics
    \end{itemize}

    \item \textbf{Computational and Financial Costs}
    \begin{itemize}
        \item GitHub Actions minutes are limited for free accounts (2,000 minutes/month)
        \item Running comprehensive quality analysis on large codebases consumes significant minutes
        \item LangChain repository has 150+ contributors with dozens of daily commits
        \item Would require paid GitHub plan (\$4-\$21/user/month) for adequate compute quota
    \end{itemize}
\end{enumerate}
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item Filter by date range (e.g., October 29 - November 27, 2025 for LangChain)
        \item Filter by state: \texttt{is:pr is:open}, \texttt{is:pr is:merged}, \texttt{is:pr is:closed}
    ...
    \begin{itemize}
        ...
        \item Identify bug-related issues using keyword search: bug, error, fix, broken, crash, fail, regression, exception
    ...
    \begin{itemize}
        ...
        \item Note relative timestamps ("2 days ago") and convert to absolute dates
    \end{itemize}
\end{enumerate}
...
The complete analysis pipeline is available in \texttt{examples/dora\_metrics.py}. The script implements all DORA metric calculations and generates visualizations.

\subsection{Key Functions}

\begin{itemize}
    \item \texttt{load\_data(filepath)}: Loads CSV data and preprocesses timestamps
    \begin{itemize}
    ...
    \item \texttt{calculate\_deployment\_frequency(merged\_df, days)}: Calculates DF metric
    \begin{itemize}
        \item Counts merged PRs per day over analysis window
        \item Computes mean, median, and weekly aggregate
        \item Classifies performance: Elite ($>$1/day), High (weekly), Medium (monthly), Low ($<$monthly)
    \end{itemize}

    \item \texttt{calculate\_lead\_time(opened\_df, merged\_df)}: Calculates LT metric
    \begin{itemize}
        ...
        \item Classifies: Elite ($<$1 day), High (1-7 days), Medium (1-4 weeks), Low ($>$1 month)
    \end{itemize}

    \item \texttt{calculate\_change\_failure\_rate(issues\_df, merged\_df)}: Calculates CFR metric
    \begin{itemize}
        \item Identifies bug issues using keyword matching on issue titles/descriptions
        \item Counts bug issues within analysis window
        \item Computes ratio: (bug issues / total deployments) $\times$ 100\%
        \item Classifies: Elite (0-15\%), High (16-30\%), Medium (31-45\%), Low ($>$45\%)
    \end{itemize}

    \item \texttt{calculate\_time\_to\_restore(issues\_df, merged\_df)}: Calculates MTTR metric
    \begin{itemize}
        ...
        \item Classifies: Elite ($<$1 hour), High ($<$1 day), Medium (1-7 days), Low ($>$1 week)
    \end{itemize}

    \item \texttt{generate\_visualizations(metrics\_dict, output\_dir)}: Creates charts
    \begin{itemize}
...
\begin{tabular}{@{}llll@{}}
\toprule
\textbf{PR \#} & \textbf{Title} & \textbf{Date Relative} & \textbf{Date Absolute} \\ \midrule
...
\#25829 & docs: Add citation format to papers & 5 days ago & 2025-11-22 \\ \bottomrule
\end{tabular}
\end{table}
...
\begin{tabular}{@{}llll@{}}
\toprule
\textbf{Issue \#} & \textbf{Title} & \textbf{Date Relative} & \textbf{Date Absolute} \\ \midrule
...
\#4502 & Regression: Tool calling broken in 0.41.0 & 1 week ago & 2025-11-20 \\ \bottomrule
\end{tabular}
\end{table}
...
\#2092 & Fix memory allocation in compiler & 1 week ago & 2025-11-25 \\ \bottomrule
\end{tabular}
\end{table}
