...
The DevOps Research and Assessment (DORA) framework defines four standardized metrics
\cite{forsgren2018accelerate, rueegger2024fully} to track across projects:
...
The OpenTelemetry Collector (otelcol-contrib) scrapes
...
The collector exports metrics to OpenObserve via OTLP HTTP protocol. The OpenObserve service stores time-series metrics and provides SQL-based querying for dashboard visualization.
Deployed on port 5080, it receives OTLP metrics data at \texttt{/api/default/} endpoint. Docker Networking connects both services via bridge network (\texttt{otel-network}) with persistent
storage (\texttt{openobserve-data} volume) for metric retention. The GitHub receiver supports flexible search queries (e.g., \texttt{org:<ORG>},
...
instrumentation. The GitHub receiver \cite{opentelemetry-github-receiver} scrapes
VCS metrics via GitHub's GraphQL and REST APIs. Kalliamvakou et al.
...
Rüegger et al. \cite{rueegger2024fully} presented a fully automated solution for
measuring DORA metrics from DevOps tooling at the individual microservice level.
Their industrial case study of 37 microservices over four weeks demonstrated that
automated DORA metrics provide significantly greater transparency than traditional
survey-based approaches, revealing performance variations between individual systems
that aggregate team metrics fail to capture.
They emphasize that automated collection enables real-time insights and continuous
improvement through data-driven decision-making on a fine-granular level.
...
The OpenObserve dashboards visualize collected metrics through six panels. Repository activity metrics are described
by the \textbf{repository count}, \textbf{top 10 repositories by change volume}, \textbf{reference count by repository}, and
\textbf{repository list with organization and url} dashboards. DORA-Aligned time series metrics are described by
the \textbf{Merge time trends over time} and \textbf{code churn trends over time} dashboards. SQL queries employ
95th percentile aggregation to filter outliers and histogram-based temporal bucketing for trend analysis.
...
This project demonstrates the feasibility of automated DORA metrics collection using
OpenTelemetry and GitHub's GraphQL API. The current implementation captures
two of the four DORA metrics—Deployment Frequency and Lead Time for Changes.


Future work will focus on two primary objectives. First, extending the GitHub receiver
configuration to scan multiple repositories within an organization, enabling portfolio-level
visibility across distributed development teams. The OpenTelemetry GitHub receiver's
search query capabilities support this scalability requirement
with minimal configuration overhead through the \texttt{org:<ORG>} configuration tag.


Second, investigating whether the remaining DORA metrics of Change Failure Rate and
Time to Restore Service can be captured through this methodology. Change Failure Rate
requires correlating deployment events with failure indicators, potentially achievable
through GitHub Actions workflow status integration or issue tracking correlation.
Time to Restore Service presents greater challenges, as it depends on production incident
detection and resolution tracking, which may require integration with external monitoring
systems or Kubernetes deployment metrics beyond GitHub's scope.
